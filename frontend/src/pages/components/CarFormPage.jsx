            ...
            <div className="flex flex-col items-center">
                <div className="w-full md:w-11/12 lg:w-9/12 xl:w-7/12">
                    <div className="flex justify-end">
                        <Link to={'/account/garage/'} className="flex gap-1 bg-gray-100 px-2 py-1 rounded-2xl shadow-md shadow-gray-400 hover:ring-2 hover:ring-primary">
                    ...
                    <form className="mt-2 p-2 gap-2 rounded-xl bg-gray-100 my-2 shadow-lg shadow-gray-400 ring-2 ring-gray-300" onSubmit={saveCar}>
                        <div className="flex md:gap-32 flex-col md:flex-row">
                            <div className="md:w-1/2">
                                <p className={styleClasses}>Name of the car:</p>
                                <input type="text" value={nameOfTheCar} onChange={ev => setNameOfTheCar(ev.target.value)} placeholder="Villám McQueen" />
                                <p className={styleClasses}>License plate:</p>
                                <input type="text" value={licensePlate} onChange={ev => setLicensePlate(ev.target.value)} placeholder="ABC-123" />
                                <p className={styleClasses}>Color:</p>
                                <input type="text" value={color} onChange={ev => setColor(ev.target.value)} placeholder="red" />
                            </div>
                            <div className="md:w-1/2">
                                <p className={styleClasses}>Model of the car:</p>
                                <input type="text" value={modelOfTheCar} onChange={ev => setModelOfTheCar(ev.target.value)} placeholder="Dodge" />
                                <p className={styleClasses}>Year of the car:</p>
                                <input type="text" value={yearOfTheCar} onChange={ev => setYearOfTheCar(ev.target.value)} placeholder="2003" />
                            </div>
                        </div>
                        ...
                        <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6 w-full">
                            {addedPhotos.length > 0 && addedPhotos.map(link => (
                                <div className="flex h-32 relative" key={link}>
                                    <Image className="rounded-2xl ring-2 ring-gray-300 w-full object-cover" src={link} />
                                    ...
                                    <button onClick={(ev) => selectAsMainPhoto(ev, link)} className="cursor-pointer absolute top-1 right-1 text-white bg-black p-1 bg-opacity-50 rounded-xl">
                                        {link === addedPhotos[0] && (
                        ...
                        <textarea value={description} onChange={ev => setDescription(ev.target.value)} placeholder="description of the car" />
