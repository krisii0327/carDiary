            ...
            <div className="w-full md:w-11/12 lg:w-9/12 xl:w-7/12 m-auto">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    {cars.length > 0 && cars.map(car => (
                        <Link to={'/garage/' + car._id} className="bg-gray-100 rounded-2xl border border-gray shadow-md shadow-gray-400 hover:ring-2 hover:ring-black" key={car._id}>
                            <div className="rounded-2xl flex justify-center p-2">
                                {car.photos?.[0] && (
                                    <Image className="rounded-2xl h-32 ring-2 ring-white shadow-md shadow-gray-400" src={car.photos?.[0]} alt="" />
                                )}
                            </div>
